 ...
 * comprasService.ts
 * ---------------------
 * CAPA DE SERVICIOS — Módulo de Compras
 ...
 *  ✅ Lee la URL base desde VITE_API_COMPRAS con fallback a localhost:4000 (desarrollo).
 */

// ─── URL base del microservicio de Compras ───────────────────────────────────────────────
// Lee VITE_API_COMPRAS del .env de Vite.
...
const API_BASE_URL =
  (import.meta.env.VITE_URL_API_COMPRAS as string | undefined) ??
  "http://localhost:4000";
...
/** Payload requerido por POST /api/compras/ingresar */
 ...
 *
 * Endpoint: GET /api/compras/stock/:idVariante
 */
export async function consultarStock(
  idVariante: number
): Promise<ConsultarStockResponse> {
  return apiFetch<ConsultarStockResponse>(
    `/api/compras/stock/${idVariante}`
  );
}

/**
 * Descuenta unidades del compras (consumido por api-ventas / ajuste manual).
 *
 ...
 *
 * Endpoint: POST /api/compras/descontar
 */
export async function descontarStock(
  idVariante: number,
  cantidad: number
): Promise<DescontarStockResponse> {
  return apiFetch<DescontarStockResponse>("/api/compras/descontar", {
    method: "POST",
 ...
 *
 * Endpoint: POST /api/compras/ingresar
 */
export async function ingresarStock(
  payload: IngresarStockPayload
): Promise<IngresarStockResponse> {
  return apiFetch<IngresarStockResponse>("/api/compras/ingresar", {
    method: "POST",
 ...
 *
 * Endpoint: GET /api/compras/sincronizar-cloud
 */
export async function sincronizarCloud(): Promise<SincronizarCloudResponse> {
  return apiFetch<SincronizarCloudResponse>("/api/compras/sincronizar-cloud");
}
...
/** Payload para POST /api/compras/tareas */
...
/** Response de POST /api/compras/tareas */
