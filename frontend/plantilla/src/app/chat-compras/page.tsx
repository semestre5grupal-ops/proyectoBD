 ...
 *  ✅ Banner "Esperando instrucciones" para OPERATIVO_COMPRAS.
 ...
 *  ✅ No fetch directo — el hook orquesta ollamaService + comprasService.
 */
...
import { useAgent, emitirVoz } from "@/app/chat-compras/hooks/use-agent"
import { MessageList } from "@/app/chat-compras/components/message-list"
import { MessageInput } from "@/app/chat-compras/components/message-input"
import { mapearRolJWT } from "@/app/chat-compras/prompts/system-prompts"
import { verificarOllama } from "@/services/ollamaService"
import type { RolCompras } from "@/app/chat-compras/types/erp-agent"
import type { Message } from "@/app/chat-compras/use-chat"
...
const MENSAJES_BIENVENIDA: Record<RolCompras, string> = {
  JEFE_COMPRAS:
    "¡Hola! Soy tu Agente ERP de Compras. Puedes darme instrucciones en lenguaje natural o por voz. Ejemplos: \"Ingresa 50 unidades del producto 12\", \"Consulta el stock del artículo 7\", \"Sincroniza el catálogo con Firebase\".",
  AUXILIAR_COMPRAS:
  ...
  OPERATIVO_COMPRAS:
    "Modo Operativo activo. Recibirás las tareas asignadas por el Jefe de Compras aquí. Confírmalas una vez ejecutadas físicamente.",
}
...
export default function ChatAgentePage() {
  ...
  const [rolOverride, setRolOverride] = useState<RolCompras | null>(null)
  ...
  const rolEfectivo: RolCompras = rolOverride ?? agent.rolActivo
  ...
  useEffect(() => {
    ...
    const apiBase = (import.meta.env.VITE_API_COMPRAS as string | undefined) ?? "http://localhost:4000"

    console.log("[Bienvenida] Consultando pendientes para id_rol:", idRol, "→", apiBase)

    // 3. Fetch al endpoint de tareas pendientes
    fetch(`${apiBase}/api/compras/tareas/pendientes`, {
      ...
      .then((json: { success: boolean; total: number; data?: Array<{ mensaje_usuario?: string; accion?: string }> } | null) => {
        ...
        if (n === 1) {
          mensajeBienvenida =
            `Buenos días ${nombreUsuario}. Tienes un mensaje pendiente en tu bandeja de compras. ` +
            `${resumen1}. ` +
            `¿Deseas confirmarlo ahora por voz?`
        } else {
          mensajeBienvenida =
            `Buenos días ${nombreUsuario}. Tienes ${n} mensajes pendientes en tu bandeja de compras. ` +
  ...
  return (
    <BaseLayout
      title="Centro de Comando IA — Compras"
      description="Agentic ERP Task Center · Módulo de Compras · Comercial JW Cóndor"
    >
      <div className="flex flex-col h-[calc(100vh-120px)] min-h-[600px] px-4 pb-4 lg:px-6 gap-0">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Módulo Compras · Comercial JW Cóndor
              </p>
            </div>
          </div>
            ...
            {!hayTokenReal ? (
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-60">
                  ...
                  <DropdownMenuItem
                    id="rol-jefe"
                    onClick={() => setRolOverride("JEFE_COMPRAS")}
                    className="gap-2 text-sm cursor-pointer"
                  >
                    <ShieldCheck size={14} className="text-primary" />
                    <div>
                      <p className="font-medium">JEFE_COMPRAS</p>
                      <p className="text-xs text-muted-foreground">
                        Todos los permisos
                      </p>
                    </div>
                  ...
                  <DropdownMenuItem
                    id="rol-auxiliar"
                    onClick={() => setRolOverride("AUXILIAR_COMPRAS")}
                    className="gap-2 text-sm cursor-pointer"
                  >
                    <ShieldAlert size={14} className="text-amber-500" />
                    <div>
                      <p className="font-medium">AUXILIAR_COMPRAS</p>
                      <p className="text-xs text-muted-foreground">
                  ...
                  <DropdownMenuItem
                    id="rol-operativo"
                    onClick={() => setRolOverride("OPERATIVO_COMPRAS")}
                    className="gap-2 text-sm cursor-pointer"
                  >
                    <ShieldOff size={14} className="text-muted-foreground" />
                    <div>
                      <p className="font-medium">OPERATIVO_COMPRAS</p>
                      <p className="text-xs text-muted-foreground">
        ...
        {/* ══════════════════════════════════════════════════════════════════
            BANNER DE OPERATIVO — solo visible para OPERATIVO_COMPRAS
        ══════════════════════════════════════════════════════════════════ */}
        {rolEfectivo === "OPERATIVO_COMPRAS" && (
          <div className="flex items-center gap-2.5 px-4 py-2.5 bg-muted/60 border-b text-sm text-muted-foreground">
            ...
            <span>
              ...
              Esperando instrucciones del Jefe de Compras. Confirma las tareas
