...
export default function PermisosPage() {
  ...
  const handleOpenModal = (per?: Permiso) => {
    if (per) {
      ...
      setFormData({
        id_empleado: "",
        per_fecha_inicio: new Date().toISOString().split('T')[0],
        per_fecha_fin: "",
        per_estado: "PEN",
        per_observacion: ""
      })
  ...
  const handleSave = async () => {
    ...
    try {
      const data: any = {
        ...
        per_observacion: formData.per_observacion || null
      }
  ...
  const handleApprove = async (per: Permiso, approve: boolean) => {
    if (!per.id_permiso) return
    try {
      const data = { ...per, per_estado: approve ? 'APR' : 'REC' }
      await permisoService.update(per.id_permiso, data)
      setPermisos(prev => prev.map(p => p.id_permiso === per.id_permiso ? data : p))
      toast.success(approve ? "Permiso aprobado" : "Permiso rechazado")
    } catch (err: any) {
      console.error(err)
      toast.error("Error: " + (err.message || "No se pudo cambiar el estado del permiso"))
    }
  }
  ...
  return (
    <BaseLayout 
      ...
      description="Administración de justificaciones y solicitudes de permisos de los empleados."
    >
      <div className="flex flex-col gap-4 px-4 lg:px-6 mt-6">
        ...
        <div className="flex justify-between items-center bg-white p-4 rounded-lg shadow-sm border">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-semibold">Solicitudes de Permiso</h2>
          </div>
        ...
        <div className="flex items-center bg-white p-1 rounded-lg shadow-sm border w-full max-w-md">
          ...
          <Input 
            placeholder="Buscar por empleado u observaciones..." 
            value={searchTerm}
            onChange={(e) => {
                ...
                {currentPermisos.length === 0 ? (
                  ...
                  currentPermisos.map((per) => (
                    <TableRow key={per.id_permiso}>
                      <TableCell className="font-medium">{getEmpleadoName(per.id_empleado)}</TableCell>
                      <TableCell>{new Date(per.per_fecha_inicio).toLocaleDateString()}</TableCell>
                      <TableCell>{new Date(per.per_fecha_fin).toLocaleDateString()}</TableCell>
                      <TableCell>
                        <span className={`inline-flex items-center rounded-full px-2 py-1 text-xs font-medium ring-1 ring-inset ${
                          per.per_estado === 'APR' 
                            ? 'bg-green-50 text-green-700 ring-green-600/20' 
                            : per.per_estado === 'REC' || per.per_estado === 'REZ'
                      ...
                      <TableCell className="max-w-xs truncate" title={per.per_observacion || ""}>
                        {per.per_observacion || "-"}
                      </TableCell>
                      <TableCell className="text-right flex justify-end gap-1">
                        {canApprove && per.per_estado === 'PEN' && (
                          <>
                            <Button size="icon" variant="ghost" title="Aprobar" onClick={() => handleApprove(per, true)}>
                              <CheckCircle size={16} className="text-green-500" />
                            </Button>
                            <Button size="icon" variant="ghost" title="Rechazar" onClick={() => handleApprove(per, false)}>
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>{editingPermiso ? "Editar Solicitud" : "Nueva Solicitud de Permiso"}</DialogTitle>
          </DialogHeader>
